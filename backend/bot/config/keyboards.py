...
def tariff_payment_keyboard(config_id: int, url: str):
    ...
    builder.row(
        ...
        InlineKeyboardButton(text="❌ Отмена", callback_data=PaymentCallback(action="cancel", config_id=config_id).pack())
    )
...
def configs_keyboard(configs: List[Config], page: int = 0):
    ...
    for config in page_configs:
        ...
        builder.row(
            InlineKeyboardButton(text=config.config_name, callback_data=ConfigCallback(action="config", config_id=config.id).pack())
        )
